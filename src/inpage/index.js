...
const INPAGE = 'eidoo-inpage'
const CONTENT_SCRIPT = 'eidoo-contentscript'
const PROVIDER = 'eidoo-provider'

// Setup stream for content script communication
const eidooStream = new ReactNativePostMessageStream({
  name: INPAGE,
...
initializeProvider({
  connectionStream: eidooStream,
  shouldSendMetadata: false
})

// Set content script post-setup function
Object.defineProperty(window, '_eidooSetupProvider', {
  value: () => {
    setupProviderStreams()
    delete window._eidooSetupProvider
  },
  configurable: true,
  enumerable: false,
  writable: false
...
function setupProviderStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) => logStreamDisconnectWarning('Eidoo Inpage Multiplex', err))
  pump(appMux, appStream, appMux, (err) => {
    logStreamDisconnectWarning('Eidoo Background Multiplex', err)
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (err) =>
    logStreamDisconnectWarning(`Eidoo muxed traffic for channel "${channelName}" failed.`, err)
  )
}
...
function logStreamDisconnectWarning(remoteLabel, err) {
  let warningMsg = `EidooContentscript - lost connection to ${remoteLabel}`
  if (err) {
    warningMsg += `\n${err.stack}`
  }
 ...
 * Relies on @eidoo/object-multiplex and post-message-stream implementation details.
 */
